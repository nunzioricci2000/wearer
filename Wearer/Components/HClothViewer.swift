                    ...
                    ForEach(clothes) { cloth in
                        if clothType.lowercased() == cloth.type?.lowercased() {
                            NavigationLink(destination: {
                                ClothEditorView(cloth: cloth, image: UIImage(data: cloth.picture ?? UIImage(imageLiteralResourceName: "ClothPlaceholder").pngData()!), type: cloth.type ?? "Unknown", warmIndex: Int(cloth.warmness))
                            }, label: {
